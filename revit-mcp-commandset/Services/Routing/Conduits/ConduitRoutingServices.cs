...
namespace RevitMCPCommandSet.Services.Routing.Conduits
{
    public static class ConduitRoutingServices
    {
        ...
        // --- Step3: 推斷/取得必要資訊
        public static ConduitRoutingContext InferRoutingContext(Document doc, Element s, Element e, ConduitRouteTask task)
        {
            ...
            try
            {
                ...
                // 推論時由 task 輸入，後續由 endpoint 元素決定
                ctx.DiameterFt = task.ConduitDiameterMm;
                // 直接採用預設值
                ...
                {
                    if (task.Override.ConduitTypeId.HasValue) ctx.ConduitTypeId = new ElementId(task.Override.ConduitTypeId.Value);
                    if (task.Override.LevelId.HasValue) ctx.LevelId = new ElementId(task.Override.LevelId.Value);
                    if (task.Override.DiameterMm.HasValue) ctx.DiameterFt = task.Override.DiameterMm.Value / 304.8;
                }

                if (ctx.ConduitTypeId == null|| ctx.LevelId == null)
                    throw new InvalidOperationException("無法推斷必要的 ConduitType/Level");

                if (ctx.DiameterFt <= 0)
                    throw new InvalidOperationException("無法推斷管徑");

                _logger.Info($"[InferCtx][OUT] , ConduitType={ctx.ConduitTypeId}, Level={ctx.LevelId}, Dia={ctx.DiameterFt * 304.8:F1}mm, Tol={ctx.ToleranceFt * 304.8:F1}mm");
                return ctx;
            }
            catch (Exception ex)
            {
